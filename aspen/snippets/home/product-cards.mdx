export const ProductCard = ({ img, href, title, description, model, type }) => {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="group border-none">
      <div className="flex flex-col gap-4 rounded-xl md:p-1">
        <div className="w-full h-36 overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
          <img
            src={img}
            className="w-full h-full object-cover transition-transform scale-105 duration-300 group-hover:scale-110 m-0"
            alt={title}
            loading="lazy"
          />
        </div>
